...
const TimelineView: React.FC<TimelineViewProps> = ({ tasks, projects, users, onTaskClick }) => {
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    return filtered;
  }, [tasks, sortBy, filterStatus, filterPriority, projects]);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select 
              ...
              className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gov-300"
            >
      ...
      {/* Legend */}
      <div className="p-3 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex gap-6">
        ...
        <div className="ml-auto text-slate-400">
          Click on task bars to view details • Scroll horizontally to see more dates
        </div>
      </div>
    </div>
  );
