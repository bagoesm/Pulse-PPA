...
export const ShareProjectModal: React.FC<ShareProjectModalProps> = ({
  ...
  const handleRegenerateToken = async () => {
    ...
    try {
      ...
      if (data) {
        onShareStateChanged(data as ProjectDefinition);
        showNotification('Tautan Diperbarui', 'Tautan lama tidak berlaku lagi, tautan baru telah dibuat.', 'success');
        setCopied(false);
      }
    } catch (error: any) {
            ...
            <div className="space-y-5">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      readOnly
                      value={shareUrl}
                      className="w-full pl-3 pr-10 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-mono text-slate-600 select-all focus:outline-none focus:ring-1 focus:ring-gov-500"
                    />
                    <Link2 className="absolute right-3 top-2.5 text-slate-400" size={14} />
                  </div>
                  <button
                    onClick={handleCopy}
                    className="flex items-center gap-1.5 px-3 py-2 bg-gov-600 text-white rounded-lg font-semibold text-xs hover:bg-gov-700 transition-colors shadow-sm"
                  >
                    {copied ? (
                      <>
                        <Check size={14} />
              ...
              <div className="pt-2 flex flex-col gap-2">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 hover:border-slate-300 text-slate-700 rounded-lg font-semibold text-xs hover:bg-slate-50 transition-colors"
                >
                  {isLoading ? (
                  ...
                  <span>Buat Ulang Tautan Baru</span>
                </button>
